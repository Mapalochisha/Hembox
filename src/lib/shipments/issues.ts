...
export const SHIPMENT_ISSUE_TYPES = ["DELAYED", "LOST", "DAMAGED", "WRONG_ITEM", "ADDRESS_PROBLEM", "COURIER_PROBLEM", "CUSTOMER_UNAVAILABLE", "OTHER"] as const;
export const SHIPMENT_ISSUE_RESPONSIBILITIES = ["UNASSIGNED", "COURIER", "STORE", "CUSTOMER", "THIRD_PARTY"] as const;
export const SHIPMENT_ISSUE_STATUSES = ["OPEN", "INVESTIGATING", "RESOLVED", "CLOSED"] as const;
...
export function normalizeIssueText(value: string | null | undefined): string | null { if (value === undefined || value === null) return null; const normalized = value.trim(); return normalized || null; }

export function validateIssueUpdate(input: { status?: string; responsibility?: string; description?: string; resolution?: string | null; note?: string | null; }): string | null {
...
export async function listShipmentIssues(): Promise<ShipmentIssueRecord[]> {
  return db.$queryRaw<ShipmentIssueRecord[]>(Prisma.sql`
    SELECT i.id, i.shipment_id AS "shipmentId", i.type, i.description, i.responsibility, i.status, i.resolution,
      ...
      s."trackingNumber" AS "trackingNumber", o."orderNumber" AS "orderNumber"
...
export async function listShipmentIssueEvents(issueId: string) {
  return db.$queryRaw<unknown[]>(Prisma.sql`
    SELECT id, admin_email AS "adminEmail", from_status AS "fromStatus", to_status AS "toStatus", responsibility, note, resolution, created_at AS "createdAt"
    FROM shipment_issue_events WHERE issue_id = ${issueId} ORDER BY created_at DESC
  `);
}

export async function createShipmentIssue(input: { shipmentId: string; type: ShipmentIssueType; description: string; responsibility?: ShipmentIssueResponsibility; note?: string | null; adminEmail?: string | null; }) {
  ...
  const issueId = id(); const eventId = id(); const responsibility = input.responsibility ?? "UNASSIGNED"; const now = new Date();
  await db.$transaction(async (tx) => {
    await tx.$executeRaw(Prisma.sql`INSERT INTO shipment_issues (id, shipment_id, type, description, responsibility, status, created_by_email, created_at, updated_at) VALUES (${issueId}, ${input.shipmentId}, ${Prisma.raw(`'${input.type}'::"ShipmentIssueType"`)}, ${description}, ${Prisma.raw(`'${responsibility}'::"ShipmentIssueResponsibility"`)}, 'OPEN'::"ShipmentIssueStatus", ${input.adminEmail ?? null}, ${now}, ${now})`);
    await tx.$executeRaw(Prisma.sql`INSERT INTO shipment_issue_events (id, issue_id, admin_email, to_status, responsibility, note, created_at) VALUES (${eventId}, ${issueId}, ${input.adminEmail ?? null}, 'OPEN'::"ShipmentIssueStatus", ${Prisma.raw(`'${responsibility}'::"ShipmentIssueResponsibility"`)}, ${normalizeIssueText(input.note)}, ${now})`);
  });
...
export async function updateShipmentIssue(input: { issueId: string; status?: ShipmentIssueStatus; responsibility?: ShipmentIssueResponsibility; description?: string; resolution?: string | null; note?: string | null; adminEmail?: string | null; }) {
  ...
  const now = new Date(); const eventId = id(); const resolved = nextStatus === "RESOLVED" || nextStatus === "CLOSED";
  await db.$transaction(async (tx) => {
    await tx.$executeRaw(Prisma.sql`UPDATE shipment_issues SET description = ${nextDescription}, responsibility = ${Prisma.raw(`'${nextResponsibility}'::"ShipmentIssueResponsibility"`)}, status = ${Prisma.raw(`'${nextStatus}'::"ShipmentIssueStatus"`)}, resolution = ${nextResolution}, resolved_by_email = CASE WHEN ${resolved} THEN ${input.adminEmail ?? null} ELSE resolved_by_email END, resolved_at = CASE WHEN ${resolved} THEN COALESCE(resolved_at, ${now}) ELSE NULL END, updated_at = ${now} WHERE id = ${input.issueId}`);
    await tx.$executeRaw(Prisma.sql`INSERT INTO shipment_issue_events (id, issue_id, admin_email, from_status, to_status, responsibility, note, resolution, created_at) VALUES (${eventId}, ${input.issueId}, ${input.adminEmail ?? null}, ${Prisma.raw(`'${String(current.status)}'::"ShipmentIssueStatus"`)}, ${Prisma.raw(`'${nextStatus}'::"ShipmentIssueStatus"`)}, ${Prisma.raw(`'${nextResponsibility}'::"ShipmentIssueResponsibility"`)}, ${normalizeIssueText(input.note)}, ${nextResolution}, ${now})`);
  });
  return getShipmentIssue(input.issueId);
}

export function shipmentStatusCanHaveIssue(status: ShipmentStatus): boolean { return status !== ShipmentStatus.CANCELLED; }
