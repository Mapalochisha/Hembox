        ...
        {["shipping", "review"].map((s, i) => (
          <div key={s} className="flex items-center gap-3">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black
              ${step === s || (s === "shipping" && step === "review") ? "bg-[#111] text-white" : "bg-gray-100 opacity-40"}`}>
              {i + 1}
            </div>
            ...
            <form onSubmit={handleShippingSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select name="province" value={form.province} onChange={handleChange}
                    className="w-full border border-gray-200 rounded px-4 py-3 text-sm focus:outline-none focus:border-[#111] transition-colors bg-white">
              ...
              <div>
                ...
                <textarea name="notes" value={form.notes} onChange={handleChange} rows={3}
                  className="w-full border border-gray-200 rounded px-4 py-3 text-sm focus:outline-none focus:border-[#111] transition-colors resize-none" />
              </div>

              {error && <p className="text-red-500 text-xs">{error}</p>}

              <button type="submit"
                className="w-full bg-[#111] text-white text-xs tracking-widest uppercase font-bold py-4 rounded hover:bg-black transition-colors">
                Continue to Review
              </button>
            </form>
          )}
          ...
          {step === "review" && (
            <div>
              ...
              <button onClick={handlePlaceOrder} disabled={loading}
                className={`w-full text-white text-xs tracking-widest uppercase font-bold py-4 rounded transition-colors
                  ${loading ? "bg-gray-400 cursor-not-allowed" : "bg-[#111] hover:bg-black"}`}>
                {loading ? "Placing Order..." : "Place Order"}
              </button>
              <button onClick={() => setStep("shipping")}
                className="w-full text-xs tracking-widest uppercase font-bold py-3 text-center border border-gray-200 rounded hover:bg-gray-50 transition-colors mt-3">
                Back to Shipping
              </button>
            </div>
          )}
