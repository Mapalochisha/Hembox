...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const rate = await db.shippingRate.findUnique({ where: { id: params.id }, include: { deliveryZone: { include: { courier: true } }, packageTier: true } });
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const data: Prisma.ShippingRateUncheckedUpdateInput = {
      deliveryZoneId,
      packageTierId,
      courierCost: courierCost ?? undefined,
    ...
    if (error?.code === "P2025") return NextResponse.json({ error: "Shipping rate not found." }, { status: 404 });
    return NextResponse.json({ error: error?.message ?? "Failed to update shipping rate." }, { status: 400 });
  }
}
