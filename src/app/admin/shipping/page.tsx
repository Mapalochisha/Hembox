    ...
    <div className="space-y-6">
      ...
      {loading ? <div className="rounded-xl border bg-white p-8 text-sm text-gray-500">Loading shipping configuration…</div> : (
        <>
          ...
          {tab === "Zones" && <section className="space-y-5"><CreateCard title="Add delivery zone" onSubmit={createZone} saving={saving}><Select name="courierId" label="Courier" options={activeCouriers.map((c) => [c.id, `${c.name} (${c.code})`])}/><Field name="code" label="Code" placeholder="LUSAKA"/><Field name="name" label="Name" placeholder="Lusaka"/><Field name="description" label="Description" placeholder="Optional description"/><p className="text-xs text-gray-500">After creating a zone, open its details to assign active delivery locations.</p></CreateCard><Table headers={["Courier", "Code", "Name", "Locations", "Rates", "Status", ""]}>{zones.map((z) => <tr key={z.id}><Cell>{z.courier.name}</Cell><Cell>{z.code}</Cell><Cell><Link href={`/admin/shipping/zones/${z.id}`} className="font-medium text-gray-900 hover:underline">{z.name}</Link></Cell><Cell>{z._count?.locations ?? 0}</Cell><Cell>{z._count?.rates ?? 0}</Cell><Cell><Status active={z.isActive}/></Cell><Cell><div className="flex items-center gap-3"><Link href={`/admin/shipping/zones/${z.id}`} className="text-sm font-medium text-gray-700 hover:underline">Details</Link><button disabled={saving} onClick={() => void submit(`/api/admin/shipping/zones/${z.id}`, "PATCH", { isActive: !z.isActive })} className="text-sm font-medium hover:underline">{z.isActive ? "Deactivate" : "Activate"}</button></div></Cell></tr>)}</Table></section>}
